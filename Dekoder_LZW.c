...
void LZW_DECODE(char* table[],char filename_read[],char filename_write[]) {
     ...
     char* string=NULL,*character=malloc(sizeof(char));
     ...
     while((new_code = ReadBits(f, code_length)) != -1) {
          ...
          if (table[new_code]==NULL) {
               temp=realloc(string,(strlen(table[old_code])+2)*sizeof(char));
               if (temp==NULL) {
                    free(temp);
                    fclose(f);
                    fclose(out);
               ...
               temp=realloc(string,(strlen(table[new_code])+1)*sizeof(char));
               if (temp==NULL) {
                    free(temp);
                    fclose(f);
                    fclose(out);
